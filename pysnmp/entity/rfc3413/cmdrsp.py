...
class GetCommandResponder(CommandResponderBase):
    ...
    def handleMgmtOperation(self, snmpEngine, stateReference, contextName, PDU, acInfo):
        (acFun, acCtx) = acInfo
        # rfc1905: 4.2.1.1
        mgmtFun = self.snmpContext.getMibInstrum(contextName).readMibObjects
        self.sendVarBinds(
            snmpEngine,
            stateReference,
            0,
...
class NextCommandResponder(CommandResponderBase):
    ...
    def handleMgmtOperation(self, snmpEngine, stateReference, contextName, PDU, acInfo):
        (acFun, acCtx) = acInfo
        # rfc1905: 4.2.2.1
        mgmtFun = self.snmpContext.getMibInstrum(contextName).readNextMibObjects
        varBinds = v2c.apiPDU.getVarBinds(PDU)
        while True:
...
class SetCommandResponder(CommandResponderBase):
    ...
    def handleMgmtOperation(self, snmpEngine, stateReference, contextName, PDU, acInfo):
        (acFun, acCtx) = acInfo
        mgmtFun = self.snmpContext.getMibInstrum(contextName).writeMibObjects
        # rfc1905: 4.2.5.1-13
        try:
            self.sendVarBinds(
                snmpEngine,
