...
class CommandResponderBase:
    ...
    def completeMgmtOperation(self, varBinds, **context):
        (errorIndication, errorStatus, errorIndex, varBinds) = self._mapSmiErrors(
            varBinds, **context
        )
        ...
        self.sendVarBinds(
            context["snmpEngine"], stateReference, errorStatus, errorIndex, varBinds
        )
...
class NextCommandResponder(CommandResponderBase):
    ...
    def _getManagedObjectsInstances(self, varBinds, **context):
        ...
        for idx, varBind in enumerate(varBinds):
            name, val = varBind
            if exval.noSuchObject.isSameTypeWith(
                val
...
class BulkCommandResponder(NextCommandResponder):
    ...
    def _completeNonRepeaters(self, varBinds, **context):
        ...
        if rspVarBinds:
            ...
            if context["counters"]["M"] and context["counters"]["R"]:
                ...
                CommandResponderBase.completeMgmtOperation(
                    self, context["allVarBinds"], **context
                )
    ...
    def completeMgmtOperation(self, varBinds, **context):
        ...
        if rspVarBinds:
            ...
            eom = all(
                exval.endOfMibView.isSameTypeWith(value) for name, value in rspVarBinds
            )
