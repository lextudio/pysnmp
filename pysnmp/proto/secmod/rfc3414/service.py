...
class SnmpUSMSecurityModel(AbstractSecurityModel):
    ...
    def _cloneUserInfo(self, snmpEngine, securityEngineID, userName):
        ...
        mibInstrumController.writeMibObjects(
            ((usmUserEntry.name + (13,) + tblIdx2, 5),)
        )
        ...
        # Activate row
        mibInstrumController.writeMibObjects(
            ((usmUserEntry.name + (13,) + tblIdx2, 1),)
        )
    ...
    def _generateRequestOrResponseMsg(
        self,
        snmpEngine,
        ...
        if securityLevel in (1, 2, 3):
            ...
            # 3.1.6a
                ...
                ) = self._timeline[securityEngineID]
    ...
    def processIncomingMsg(
        ...
        if (
            ...
            and msgAuthoritativeEngineId not in self._timeline
        ):
            ...
            try:
                ...
                try:
                    ...
                    debug.logger & debug.FLAG_SM and debug.logger(
                        ...
                        "%r msgUserName %r" % (msgAuthoritativeEngineId, msgUserName)
                    )
        ...
        if securityLevel == 0:
            ...
            debug.logger & debug.FLAG_SM and debug.logger(
                "processIncomingMsg: pick up engine boots and engine time"
            )
        ...
        if securityLevel == 3 or securityLevel == 2:
            if msgAuthoritativeEngineId == snmpEngineID:
                ...
                debug.logger & debug.FLAG_SM and debug.logger(
                    ...
                    "snmpEngineTime (%s) from LCD" % (snmpEngineBoots, snmpEngineTime)
                )
        ...
        debug.logger & debug.FLAG_SM and debug.logger(
            ...
            "securityStateReference %s" % (msgUserName, securityStateReference)
        )
